...
確定申告自動化 Codex Plugin。Python 3.11+、SQLite WAL モードで動作する。
会社員＋副業（事業所得・青色申告）の所得税・消費税確定申告をエンドツーエンドで支援する。
帳簿管理・税額計算は CLI で実行し、確定申告書等作成コーナーへの入力は Codex in Chrome が代行する。
...
| ファイルパス | 役割 |
|------------|------|
| `skills/e-tax/SKILL.md` | e-Tax 電子申告スキル（Codex in Chrome で確定申告書等作成コーナーに入力） |
