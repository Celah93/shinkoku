...
| R8.10.1〜R10.9.30 | 70% | 旧50%→70%に緩和 |
| R10.10.1〜R12.9.30 | 50% | 経過措置を延長 |
| R12.10.1〜R13.9.30 | 30% | 新設区間 |
| R13.10.1〜 | 0%（控除不可） | 完全移行 |
...
├── 買い手: 50%
├── 少額特例: R11.9.30で終了
└── 備考: 少額特例は終了するが、仕入控除の経過措置は継続

令和12年 (2030年)
├── 売り手: 特例なし
├── 買い手: R12.9まで50% / R12.10〜30%
├── 少額特例: 終了済み
└── 備考: 30%区間へ移行

令和13年 (2031年)
├── 売り手: 特例なし
├── 買い手: R13.9まで30% / R13.10〜0%（完全移行）
├── 少額特例: 終了済み
└── 備考: 仕入控除の経過措置が終了
```
...
R8大綱により当初の3段階（80%→50%→0%）から5段階（80%→70%→50%→30%→0%）に細分化された。完全移行はR13.10へ延び、R8.10〜R10.9の2年間は70%控除となる。
...
| **R11.9.30** | 少額特例の終了日（仕入控除50%の経過措置は継続） |
| **R13.9.30** | 買い手側30%控除の終了日 |
| **R13.10.1** | 免税事業者からの仕入控除完全廃止 |
