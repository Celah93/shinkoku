...
経費合計の入力とは別に、決算書の「税理士・弁護士等の報酬・料金の内訳」を入力する。
`shinkoku ledger pf-list --db-path DB --fiscal-year YEAR` の読戻しを次の項目へ対応付ける。
対象年の実画面で内訳表を確認し、未確認のセレクタや過年度の画面位置で入力しない。

| 決算書の項目 | pf明細のフィールド | 確認する意味 |
|---|---|---|
| 支払先の住所 | `payer_address` | 税理士・弁護士等の住所であり、本人の住所ではない |
| 支払先の氏名 | `payer_name` | 本人が報酬を支払った相手の氏名 |
| 本年中の報酬等の金額 | `fee_amount` | 源泉徴収後の振込額ではなく、報酬等の金額 |
| 左のうち必要経費算入額 | `expense_deduction` | 帳簿で必要経費とした額と照合する。0を未確認の代用にしない |
| 所得税等の源泉徴収税額 | `withheld_tax` | 本人が預かって納付する支払先の税額 |

`withheld_tax` はこの内訳表の源泉欄にだけ転記し、本人の所得税申告の源泉徴収税額や還付額へ加えない。
報酬の経費と預り金の仕訳は別に確認し、内訳表を入力したことで経費を二重計上しない。
例えば本人の事業源泉134,772円と税理士への支払源泉20,420円は、本人分と内訳表へそれぞれ分ける。
