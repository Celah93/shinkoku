...
確定申告の端数処理を整理する。以下の旧様式の欄番号・復興税2.1%の例は2025・2026年向け。2027年は復興1.1%と防衛1%を分子で合算し、円未満切捨てと合算端数の内訳を保持する。詳しくは `docs/income-tax-2027.md` を参照し、旧様式の欄を新年度へ流用しない。
...
| 項目 | 端数処理 | 根拠 |
|------|---------|------|
| 年金の雑所得額 | 1円未満切捨て（控除額の比例部分は切上げとして実装） | 所得税法35条、国税庁確定申告の手引き |
...
| 新旧合算上限 | 通常40,000円。令和8・9年分の23歳未満扶養親族特例では一般枠60,000円 | 租税特別措置法41条の15の5 |
| 3区分合計上限 | 120,000円（固定） | — |

実装: `-(-premium // divisor)` で切り上げ。
特例で変わるのは一般生命保険料の新契約表と新旧合算上限だけで、3区分合計上限は変わらない。
...
| 項目 | 端数処理 | 根拠 |
|------|---------|------|
| 定額法: 取得価額 × 償却率（別表第八）× 償却月数 / 12 | 普通償却費欄で1円未満切上げ | 確定申告書等作成コーナーFAQ「減価償却費の端数処理」 |
| 定額法の事業按分: 普通償却費 × 事業専用割合 | 必要経費算入額欄で1円未満切上げ | 同FAQ（作成コーナーの運用に合わせる） |
| 定率法: 期首帳簿価額 × 償却率 | 端数処理は別課題で見直し予定 | 同FAQとの整合確認が必要 |

減価償却費の端数処理には法令上の明文規定がないため、作成コーナーの運用に合わせる。定額法は青色申告決算書の欄単位で、普通償却費を切り上げてから事業按分し、必要経費算入額も切り上げる。
...
| 地方消費税（還付） | 控除不足還付税額 × 22/78、1円未満切捨て | 地方税法72条の89 |
