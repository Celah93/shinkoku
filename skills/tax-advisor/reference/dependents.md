# 扶養控除（Dependents Deduction）

令和7〜9年分（2025〜2027年課税年度）対応。

## 扶養控除の概要（所得税法第84条）

納税者に扶養親族がいる場合に適用される所得控除。

### 扶養親族の要件

以下のすべてを満たす者:

1. 配偶者以外の親族（6親等内の血族、3親等内の姻族）又は養育・養護委託された児童・老人
2. 納税者と生計を一にしていること
3. 合計所得金額が年分別の要件以下（令和7年分は58万円以下、令和8・9年分は62万円以下）
4. 青色事業専従者・白色事業専従者でないこと

## 扶養控除額の一覧

| 区分 | 年齢 | 控除額 | 備考 |
|------|------|--------|------|
| 年少扶養親族 | 16歳未満 | 0円（控除なし） | 住民税の非課税判定には算入 |
| 一般の扶養親族 | 16歳以上19歳未満 | 38万円 | |
| 特定扶養親族 | 19歳以上23歳未満 | 63万円 | 大学生相当の年齢 |
| 一般の扶養親族 | 23歳以上70歳未満 | 38万円 | |
| 老人扶養親族（同居以外） | 70歳以上 | 48万円 | |
| 老人扶養親族（同居） | 70歳以上 | 58万円 | 同居老親等 |

※ 年齢の判定は12月31日時点。

## 令和7年分の改正ポイント

### 1. 扶養親族の所得要件変更

| 項目 | 令和6年分以前 | 令和7年分〜 |
|------|-------------|-----------|
| 扶養親族の所得要件 | 48万円以下 | **58万円以下** |
| 給与収入ベース | 103万円以下 | **123万円以下** |

### 2. 特定親族特別控除の新設

令和7年分から「特定親族特別控除」が新設された。

#### 背景

従来、19歳〜22歳の子（特定扶養親族）は合計所得48万円（給与収入103万円）を超えると
一切の扶養控除が受けられなくなる「崖」があった。アルバイト収入が103万円を超えると
親の税負担が一気に増加する問題を解消するため、段階的に控除が減少する仕組みを導入。

#### 特定親族特別控除の内容

対象: 19歳以上23歳未満の扶養親族（特定扶養親族の年齢要件に該当する者）で、
合計所得金額が58万円超〜123万円以下の者。

| 親族の合計所得金額 | 控除額 |
|------------------|--------|
| 58万円超〜85万円以下 | 63万円（満額） |
| 85万円超〜90万円以下 | 61万円 |
| 90万円超〜95万円以下 | 51万円 |
| 95万円超〜100万円以下 | 41万円 |
| 100万円超〜105万円以下 | 31万円 |
| 105万円超〜110万円以下 | 21万円 |
| 110万円超〜115万円以下 | 11万円 |
| 115万円超〜120万円以下 | 6万円 |
| 120万円超〜123万円以下 | 3万円 |
| 123万円超 | 0円 |

#### 給与収入ベースでの換算

| 親族の給与収入 | 合計所得 | 控除額 |
|--------------|---------|--------|
| 〜123万円 | 〜58万円 | 63万円（通常の特定扶養控除） |
| 123万円超〜150万円 | 58万円超〜85万円 | 63万円（特定親族特別控除・満額） |
| 150万円超〜155万円 | 85万円超〜90万円 | 61万円 |
| 155万円超〜160万円 | 90万円超〜95万円 | 51万円 |
| 160万円超〜165万円 | 95万円超〜100万円 | 41万円 |
| 165万円超〜170万円 | 100万円超〜105万円 | 31万円 |
| 170万円超〜175万円 | 105万円超〜110万円 | 21万円 |
| 175万円超〜180万円 | 110万円超〜115万円 | 11万円 |
| 180万円超〜185万円 | 115万円超〜120万円 | 6万円 |
| 185万円超〜188万円 | 120万円超〜123万円 | 3万円 |
| 188万円超 | 123万円超 | 0円 |

根拠: 令和7年度税制改正大綱

## 令和8・9年分の所得要件

扶養親族の所得要件は62万円以下へ引き上げられる。19歳以上23歳未満の親族は、
62万円以下なら特定扶養控除、62万円超123万円以下なら特定親族特別控除となる。

| 親族の合計所得金額 | 控除額 | 区分 |
|---|---:|---|
| 62万円以下 | 63万円 | 特定扶養控除 |
| 62万円超〜85万円以下 | 63万円 | 特定親族特別控除 |
| 85万円超〜90万円以下 | 61万円 | 特定親族特別控除 |
| 90万円超〜95万円以下 | 51万円 | 特定親族特別控除 |
| 95万円超〜100万円以下 | 41万円 | 特定親族特別控除 |
| 100万円超〜105万円以下 | 31万円 | 特定親族特別控除 |
| 105万円超〜110万円以下 | 21万円 | 特定親族特別控除 |
| 110万円超〜115万円以下 | 11万円 | 特定親族特別控除 |
| 115万円超〜120万円以下 | 6万円 | 特定親族特別控除 |
| 120万円超〜123万円以下 | 3万円 | 特定親族特別控除 |
| 123万円超 | 0円 | 対象外 |

給与収入だけの場合、扶養親族の所得要件は給与収入136万円以下、特定親族特別控除は
136万円超197万円以下に相当する。判定は給与収入ではなく合計所得金額で行う。

根拠: 令和8年度税制改正（所得税法第2条第1項第34号・第34号の3、所得税法第84条の2）

## 扶養控除の適用判定フロー

```
Q1. 配偶者以外に親族がいるか？（6親等内の血族、3親等内の姻族）
├── No → 扶養控除なし
└── Yes → Q2へ

Q2. その親族と生計を一にしているか？
├── No → 扶養控除なし
└── Yes → Q3へ

Q3. その親族の合計所得金額は年分別要件以下か？（令和7年58万円、令和8・9年62万円）
├── Yes → Q4へ（扶養控除の適用可能性あり）
├── 年分別要件超〜123万円以下で19-22歳 → 特定親族特別控除を適用
└── 年分別要件超（19-22歳以外） → 扶養控除なし

Q4. その親族は16歳以上か？（12月31日時点）
├── No → 扶養控除なし（年少扶養親族）
└── Yes → Q5へ

Q5. 年齢区分は？
├── 19歳以上23歳未満 → 特定扶養親族: 63万円
├── 70歳以上で同居 → 同居老親等: 58万円
├── 70歳以上で別居 → 老人扶養親族: 48万円
└── その他 → 一般の扶養親族: 38万円
```

## 生計を一にするとは

「生計を一にする」の判定基準（所基通2-47）:

### 同居している場合

- 原則として「生計を一にする」と判定
- 明らかに独立した生活を営んでいる場合は除く

### 別居している場合

以下のいずれかに該当すれば「生計を一にする」:
- 勤務、修学、療養等の都合で別居しているが、生活費等を常に送金している
- 休暇には帰省する等、実質的に同一の生計を営んでいると認められる

### 具体例

| ケース | 判定 |
|--------|------|
| 大学生の子が一人暮らし、仕送りあり | 生計を一にする |
| 老親が入院中、医療費を負担 | 生計を一にする |
| 成人した子が独立して生活 | 生計を一にしない |
| 海外留学中の子、送金あり | 生計を一にする（親族関係書類・送金関係書類が必要） |

## 16歳未満の年少扶養親族

扶養控除は適用されないが、以下に影響する:

- 住民税の非課税判定（扶養人数に算入される）
- 児童手当の算定（所得制限の判定に影響する場合あり）
- 事業税の事業主控除の判定

確定申告書の第二表「住民税に関する事項」の「16歳未満の扶養親族」欄に記載する。

## 扶養控除における注意事項

### 重複適用の禁止

- 同一の者が2人以上の納税者の扶養親族になることはできない
- 夫婦のどちらの扶養にするかは有利な方を選択可能

### 確認が必要なケース

1. **大学生のアルバイト収入**: 103万円の壁を意識するが、令和7年分からは123万円に
2. **親の年金収入**: 65歳以上で年金収入158万円以下なら所得58万円以下（公的年金等控除110万円）
3. **障害のある親族**: 扶養控除と障害者控除の両方が適用可能
4. **海外居住の親族**: 親族関係書類と送金関係書類の提出が必要（30歳以上70歳未満の場合は留学等の要件あり）

## よくある間違い

1. **16歳未満への扶養控除適用**: 16歳未満は控除額0円（住民税欄への記載は必要）
2. **所得と収入の混同**: 給与収入123万円以下 ≒ 所得58万円以下（令和7年分）
3. **年齢の判定時期**: 12月31日時点の年齢で判定（年度途中ではない）
4. **重複控除**: 兄弟で同一の親を扶養に入れてしまう
5. **特定親族特別控除の見落とし**: 令和7年分から新設された制度を知らない

## 参照条文

- 所得税法第2条第1項第34号（扶養親族の定義）
- 所得税法第2条第1項第34号の2（控除対象扶養親族）
- 所得税法第84条（扶養控除）
- 所得税法第84条の2（特定親族特別控除）
- 所得税法第85条（扶養親族等の判定の時期）
- 所基通2-47（生計を一にするの意義）
- 令和7年度税制改正大綱（扶養控除の所得要件変更、特定親族特別控除の新設）
- 令和8年度税制改正（扶養親族・特定親族の所得要件を62万円へ引上げ）
