...
**令和8・9年分の注意**: 23歳未満扶養親族に係る生命保険料控除特例は所得税だけの制度で、
住民税の生命保険料控除は増えない。`calc-deductions` が返す所得税の控除合計を
`calc-furusato-limit` へ使う場合は、特例で増えた額を住民税側の控除として含めず、
住民税用の控除額へ調整してから渡す。現行の上限計算は所得税控除を住民税控除の近似に
使うため、未調整の値では上限を小さめに見積もることがある。
