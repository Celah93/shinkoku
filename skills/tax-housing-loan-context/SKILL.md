...
2. 入居年に一致する借入限度額・控除率・控除期間を提示する。単体計算は2022〜2030年入居、
   年間所得税は2025〜2027年分。将来入居の試算は `docs/housing-loan-standalone.md` を参照する
...
単体計算では次のCLIを使用する。結果は所得税額上限の適用前であり、申告税額ではない。2028年以後の新築は立地と建築日の経過措置を確認する。実帳簿への新情報の登録には対象内容のユーザー確認を得る。

```bash
shinkoku tax calc-housing-loan --input FILE
```
