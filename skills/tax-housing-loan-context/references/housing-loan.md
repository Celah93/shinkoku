...
令和4〜8年入居に対応。令和9年以降の入居は実装範囲外として計算を止める。
...
令和6年6月30日までに建築された住宅も対象になり得るが、現行入力には建築日がないため
この経路は未対応。建築確認フラグが`false`のときは0円に加えて未対応警告を返す。
40㎡以上50㎡未満の床面積判定も未対応なので、警告が出たら証拠書類を手作業で確認する。
