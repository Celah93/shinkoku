...
| 所得税サニティチェック | 計算入力と計算結果を再計算し、青色申告特別控除、課税所得の丸め、復興・防衛特別所得税、合算額、税額控除、源泉徴収税額、還付額などを検査 |
| 消費税 | 2割特例、簡易課税、本則課税の計算・比較、税率区分、インボイス区分、経過措置、帳簿のみ保存の特例、申告書第二表用の集計、警告の出力 |
| ふるさと納税 | 寄附記録の追加・一覧・削除・集計、寄附金控除、住民税に基づく上限推定と2027年の193万円特例控除上限 |
| 住民税控除の組立て | 2025〜2027年の控除元データから標準所得割を推定し、既存のふるさと納税上限計算へ接続。[入力と確認事項](docs/resident-tax-estimate.md) |
| 住宅ローン単体計算 | 2022〜2030年入居の控除可能額、建築日の経過措置・立地・面積・期間を確認。年間所得税の対応年分は2027年までを維持。[単体計算の範囲](docs/housing-loan-standalone.md) |
| 高所得特例 | 基準所得・初回判定・再計算・加算所得税と追加税の計算。対象外の所得区分の通常税額・申告書作成は別途必要 |
...
所得税計算・控除集計・ふるさと納税の上限推定は2025〜2027年分に対応します。2027年の上限推定は住民税の入力項目が必要です。高所得特例の二段階判定・加算計算も追加し、対象となる所得は年間計算へ反映します。[入力と対応範囲](docs/furusato-and-minimum-tax-2027.md)を確認してください。消費税は本則・簡易・2割特例が2025・2026年分、国内の個人事業者の3割特例が2027・2028年分に対応します。3割特例の相続の例外判定と、国税・地方税の中間納付精算にも対応します。2027年の新制度を含む作成コーナー入力は未検証です。[2027年分の計算と残る範囲](docs/income-tax-2027.md)を参照してください。機能ごとの年分は[対応範囲](docs/tax-year-support.md)を確認してください。

所得税・消費税の従来入力は `estimate`（試算）として扱います。申告用計算では `calculation_mode: "filing"` を指定し、青色控除・2割特例・3割特例の必要な事実が未確認又は不適格なら停止します。[適用判定と入力例](docs/tax-eligibility.md)を確認してください。これらの判定だけで、他の控除や申告全体の適格性を確認済みとは扱いません。
...
| 公的年金 | 単体計算と年間所得税への統合（給与との控除上限・所得金額調整を含む） | 年金源泉徴収票からの自動連携、e-Tax 入力の検証 |
...
通常はメインエージェントが原本を読み取り、主要項目を照合します。ユーザーがその会話でサブエージェント利用を明示的に依頼し、環境が対応している場合だけ、2つの独立した読み取りを比較します。判読不能な項目はまとめて確認し、実帳簿への登録前に内容を確認します。
